...
def handle_resume():
    ...
    # 加载 checkpoint (固定路径 results/.checkpoint.pt)
    ...
    if not ckpt_data:
        ...
        print()
        print("提示:")
        print("  - checkpoint 位置: results/.checkpoint.pt")
    ...
    print("📂 找到 checkpoint: results/.checkpoint.pt")
    ...
    # 保存结果 - 使用恢复的输出选项
    output_dir = Path(f"results/alpha_scan/{config.experiment_name}")
