...
time_eig = nan;
if isa(A,'numeric')||isa(A,'gpuArray')
...
if isa(opt.fxnErr3,'function_handle')
    fxnErr3 = opt.fxnErr3;
else
    fxnErr3 = [];
end
...
while ~stop
    ...
    if ~silent
      if any(gam2z<gamMin), 
        ...
        warning('gam2z=%g too small at column %d, iter=%d\n',...
                gather(gam2z(bad)),gather(bad),i*ones(size(bad)));
    ...
    if ~silent
      if any(gam2x<gamMin), 
        ...
        warning('gam2x=%g too small at column %d, iter=%d\n',...
                gather(gam2x(bad)),gather(bad),i*ones(size(bad)));
  ...
  if ~isempty(fxnErr3)
      err3(:,i) = fxnErr3(p1,gam1z,z1,eta1z,p2,gam2z,...
                          r1,gam1x,x1,eta1x,r2,gam2x,...
                          x2,z2,eta2x,eta2z).'; % evaluate 3rd error function
  end

  if verbose
    string = 'iter=%3i, col=%3i: eta2x/eta1x=%8.5f, eta2z/eta1z=%8.5f';
    data = [i*ones(L,1),(1:L).',(eta2x./eta1x).',(eta2z./eta1z).'];
    if ~isempty(fxnErr1), 
      string = [string,', err1=%8.5g']; 
      data = [data,err1(:,i)];
    end
    if ~isempty(fxnErr2), 
      string = [string,', err2=%8.5g']; 
      data = [data,err2(:,i)];
    end
    if ~isempty(fxnErr3), 
      string = [string,', err3=%8.5g']; 
      data = [data,err3(:,i)];
    end
    fprintf([string,'\n'],data.');
  end
    ...
    if ~silent
      if any(gam1x<gamMin),
        ...
        warning('gam1x=%g too small at column %d, iter=%d\n',...
                gather(gam1x(bad)),gather(bad),i*ones(size(bad)));
    ...
    if ~silent
      if any(gam1z<gamMin), 
        ...
        warning('gam1z=%g too small at column %d, iter=%d\n',...
                gather(gam1z(bad)),gather(bad),i*ones(size(bad)));
...
if saveHist
    ...
    estHist.err3 = estHist.err3(:,iTrim);
end
