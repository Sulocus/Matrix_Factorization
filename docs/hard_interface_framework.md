...
- `algorithm_params.seed_partition_policy` 为 tensor parallel 提供 opt-in 新路径：默认 `legacy` 保持旧数值；`partition_invariant` 会把 tensor supergraph index 与 student initialization 的随机流按 alpha/sample/dimension/role 分开，`resource_plan.seed_policy` 会显示 `partition_invariant=true` 和 `automatic_rebatch_allowed=true`。这只是 tensor parallel v1，不代表 spreading/dense/AGD 已经可自动重分批。
