...
- `algorithm_params.seed_partition_policy` 提供 opt-in 新路径：默认 `legacy` 保持旧数值；`partition_invariant` 会让 tensor parallel 的 tensor supergraph/F/student initialization 按 alpha/sample/dimension/role 分流，也会让 AGD 与 dense BigAMP 的 student initialization 按 alpha/sample/role 分流。`resource_plan.seed_policy` 和 runtime metadata 会显示 `partition_invariant=true` 与 `automatic_rebatch_allowed=true`。spreading 仍未完成这一改造，不能自动重分批。
