...
- precision 已迁移到 `algorithm_params.precision_profile`：`safe / fast / aggressive`。旧 `use_bf16` 与 `dtype_fallback_policy` 仍作为 compatibility alias，但新配置应写 `precision_profile` 与 `precision_fallback_policy`。execution metadata 会记录 `precision_profile`、`dtype_status`、effective BF16/TF32，以及 normalization schema v4。
...
- resource/optimization 参数也按真实 consumer 标记。`precision_profile` 现在会进入 dense BigAMP、spreading、tensor、AGD 和 memory estimator；legacy `use_bf16` 只作为 alias 解释。AGD 的 `use_compile` 仍属于 inactive route，会在普通 validate 中 warning、在 strict mode 中 error。
