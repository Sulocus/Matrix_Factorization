...
- tensor parallel 新增 `algorithm_params.dtype_fallback_policy`：默认 `allow` 保持旧行为，BF16 请求在不支持的设备上回到 FP32；设为 `error` 时直接报错。`tensor_execution.dtype_status` 会记录 `bf16_requested_and_effective`、`bf16_disabled_by_config` 或 `fallback_to_float32_bf16_unavailable`。
