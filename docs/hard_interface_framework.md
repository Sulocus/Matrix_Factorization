...
- runtime hooks 已先接入低风险 runner-level `after_batch`：新增 `batch_summary` probe，可记录 batch index、alpha values、metric keys 和 AlgorithmResult 可用输出。它不进入算法 step loop，因此不改变训练状态、随机数或数值行为。
