...
- `Q_W_COS_ROOT` / `Q_X_COS_ROOT`：sqrt baseline-corrected Gram diagnostic。

## 仍需人工确认

- `Q_Y` 的论文图例是否只显示 `Q_Y`，还是在多空间图中显示 `Q_Y^{obs}`、`Q_Y^{unobs}`、`Q_Y^{full}`。
- `Q_W/Q_X` 在 gauge/rotation 敏感时是否默认放进主图，还是默认只画 `Q_W_COS_ROOT/Q_X_COS_ROOT`。
- `Q_N` 是否未来也需要 Cos-root 或 permutation-invariant diagnostic。
...
- `Q_W_prime/Q_X_prime` 是旧 baseline-corrected Gram 名，active schema 已迁移到 `Q_W_COS_ROOT/Q_X_COS_ROOT`。
