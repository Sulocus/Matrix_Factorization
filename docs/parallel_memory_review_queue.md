...
- tensor parallel、dense BigAMP 和 AGD 已有 opt-in `algorithm_params.seed_partition_policy=partition_invariant` v1；默认 legacy 不变。
- spreading 仍未完成 partition-invariant seed 改造。

需要确认：

- spreading 是否也需要 `seed = hash(base_seed, alpha, sample, role)` 这类 partition-invariant seed policy。
