...
| 文件 | 职责 |
|------|------|
| `MF/config.yaml` | 唯一配置入口，定义所有实验参数 |
| `MF/cli.py` | CLI 入口，解析 YAML 并构建 `ExperimentConfig` |
| `MF/core/experiment/config.py` | 定义 `AlgorithmParams`, `SpreadingConfig` 等数据类 |
| `MF/core/experiment/runner.py` | 实验执行引擎，调用算法并收集结果 |
| `MF/core/experiment/data_factory.py` | 创建 Teacher 矩阵和 SpreadingData |
| `MF/modules/algorithms/bigamp_spreading.py` | 核心算法实现，包含三个训练循环 |
| `MF/modules/teachers/random_spreading.py` | 定义 `SpreadingDataParallel` 数据结构 |
| `MF/modules/outputs/plotting.py` | 结果绘图模块 |
...
**文件**: `MF/core/experiment/config.py`
...
**文件**: `MF/modules/algorithms/bigamp_spreading.py`
...
创建新测试文件 `MF/tests/test_teacher_init.py`：
...
python -m pytest MF/tests/test_teacher_init.py -v
```
...
1. 创建配置文件 `MF/presets/hysteresis_cold.yaml`:
   ```yaml
   algorithm_params:
     init_mode: random
   alpha_scan:
...
2. 创建配置文件 `MF/presets/hysteresis_warm.yaml`:
   ```yaml
   algorithm_params:
     init_mode: teacher
     init_overlap: 0.99
...
3. 运行对比：
   ```bash
   smf MF/presets/hysteresis_cold.yaml
   smf MF/presets/hysteresis_warm.yaml
   ```
...
1. **验证配置加载**:
   ```bash
   smf MF/config.yaml --dry-run  # (如果支持) 打印解析后的配置
   ```
