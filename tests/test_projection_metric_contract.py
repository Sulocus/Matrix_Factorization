...
def test_spreading_perfect_teacher_has_observed_projection_one():
    ...
    assert metrics["Q_W"] == pytest.approx(1.0)
    assert metrics["Q_X"] == pytest.approx(1.0)
...
def test_tensor_projection_and_qn_hand_fixture():
    ...
    assert qn_modes == pytest.approx([2.0, 1.0, 1.0])
    assert sum(qn_modes) / len(qn_modes) == pytest.approx(4.0 / 3.0)
