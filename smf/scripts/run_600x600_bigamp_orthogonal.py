#!/usr/bin/env python3
"""
Run 600x600 BiG-AMP experiment with Orthogonal Teacher.
Q_Y and Q_Y_unobserved metrics.

Parameters:
- N = 600, M = 150, S = 20
- Steps = 4000
- Alpha: 0.0 ~ 4.0
- Orthogonal Teacher + BiG-AMP Algorithm
"""
...
def run_600x600_orthogonal():
    """Run BiG-AMP N=600, M=150, S=20 with Orthogonal Teacher."""
    print("\n" + "="*80)
    print("600x600 BiG-AMP (Orthogonal) with Q_Y and Q_Y_unobserved")
    print("="*80)
    ...
    output_dir = Path("smf/Replica_results") / f"bigamp_orthogonal_600x600_M{M}_S{S}_steps{steps}_{timestamp}"
    ...
    config = Config(
        ...
        training=TrainingConfig(max_steps=steps, samples_per_alpha=S, seed=48),
        algorithm=AlgorithmConfig(damping=0.5, noise_var=1e-10),
        algorithm_key="bigamp",
        teacher_key="orthogonal",  # <-- Orthogonal Teacher
    ...
    ax.set_title(f'Q_Y Evolution - BiG-AMP Orthogonal (N={N}, M={M}, S={S})')
